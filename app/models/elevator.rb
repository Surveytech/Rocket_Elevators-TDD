...
class Elevator < ApplicationRecord
    ...
    def slack_message
        ...
        SlackNotifier::MESSAGE.ping "The Elevator #{self.id} with Serial Number #{self.serial_number} changed status from #{self.status_was} to #{self.status} at #{timestamp}."    #message to Slack channel via CSL Bot (WebHook)
    end
end